...
- 一覧の上の「表示するレベル」で居住〜未踏のボタンを押すと、押したレベルの島だけが一覧に出ます（複数選べます。どれも押していないときは全部の島が出ます）。
...
本州・北海道・四国・九州・沖縄本島を除く有人島432島です。人口は令和2年国勢調査の人数です。離島振興法の島は国土交通省の一覧の値、それ以外の島は国勢調査の基本単位区の人口を島ごとに足した値です。面積は、離島振興法の島は国土交通省の一覧の値、それ以外の島は国土地理院の面積調の値（1km²以上の島）、面積調に無い沖縄の島は沖縄県「離島関係資料」の値です。分からない島は「-」と表示します。令和2年国勢調査で住民が0人だった島は「0人※」と表示し、※にマウスを重ねるか※を押すと、それより前の国勢調査や住民基本台帳で住民が記録されていたことを島ごとに表示します（`scripts/census_zero_notes.json`）。
...
| 「0人※」の島の※の説明 | 総務省統計局「平成27年国勢調査」小地域集計 第1表 男女別人口及び世帯数－基本単位区（e-Stat）、周南市「周南市統計書」町名別世帯数及び男女別人口、沖縄県「離島関係資料」（令和6年3月、平成28年） | 島ごとの数値のみ使用（`scripts/census_zero_notes.json`）。e-Stat は政府標準利用規約（第2.0版）に基づき加工して作成 |
...
`web/public/google753bd16ea4a744a4.html` は Google Search Console の所有権確認用のファイルです。消すと確認が外れます。`web/public/sitemap.xml` は Search Console に送るサイトマップです。
...
| `scripts/census_zero_notes.json` | 令和2年国勢調査で0人の島の※に出す説明（それより前の国勢調査・住民基本台帳の人口）。`build_stats.mjs` は0人の島に説明が無いと止まるので、その島の説明を足してから再実行します |
